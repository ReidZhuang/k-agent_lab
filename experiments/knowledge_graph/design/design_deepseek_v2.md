...
| 属性名 | 类型 | 说明 |
| :--- | :--- | :--- |
| `relevance_score` | float | 该字段对该概念的重要性（0~1） |
...
- 遍历 DataField，生成 Embedding 向量并写入节点属性
- 遍历 IntentConcept，生成 Embedding 向量并写入节点属性
- 两两计算 DataField 余弦相似度，对相似度 ≥ 0.75 的创建 SEMANTIC_SIMILAR_TO 关系
- 根据 DataField.belongs_to_concepts 创建 BELONGS_TO_CONCEPT 关系
