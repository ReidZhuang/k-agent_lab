# daily_info — 交易日历信息（Daily Info）

## 数据源名称
- **中文名称**：交易日历信息
- **英文名称**：Daily Info
- **数据源ID**：DS_TUSHARE_DAILY_INFO

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.daily_info(trade_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，daily_info 接口提供 交易日历信息 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.daily_info(参数1=值1, 参数2=值2)
```
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.daily_info(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.daily_info(...) 返回的 DataFrame
# 示例：columns = ['exchange', 'tr', 'pe', 'trans_count', 'vol'] (前5列)
# 实际数据取决于调用参数和当前日期
```
