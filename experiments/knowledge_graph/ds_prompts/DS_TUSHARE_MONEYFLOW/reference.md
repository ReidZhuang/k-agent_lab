# moneyflow — 个股资金流向（Money Flow）

## 数据源名称
- **中文名称**：个股资金流向
- **英文名称**：Money Flow
- **数据源ID**：DS_TUSHARE_MONEYFLOW

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.moneyflow(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，moneyflow 接口提供 个股资金流向 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.moneyflow(参数1=值1, 参数2=值2)
```
...
| 字段名 | 类型 | 说明 | 数据示例 |
|:------|:----:|:-----|:--------|
| buy_elg_amount | float | buy_elg_amount | ? |
| buy_lg_amount | float | buy_lg_amount | ? |
| buy_md_amount | float | buy_md_amount | ? |
| buy_sm_amount | float | buy_sm_amount | ? |
| buy_elg_amount | float | buy_elg_amount | ? |
| buy_lg_amount | float | buy_lg_amount | ? |
| buy_md_amount | float | buy_md_amount | ? |
| buy_sm_amount | float | buy_sm_amount | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.moneyflow(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.moneyflow(...) 返回的 DataFrame
# 示例：columns = ['buy_elg_amount', 'buy_lg_amount', 'buy_md_amount', 'buy_sm_amount', 'buy_elg_amount'] (前5列)
# 实际数据取决于调用参数和当前日期
```
