...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，hibor 接口提供 Hibor利率 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.hibor(ts_code='000001.SZ')
print(df.head(10))
```

## 调用返回值样例
```
# pro.hibor(...) 返回的 DataFrame
# 示例：columns = ['on', '1w', '3m', '1m'] (前5列)
# 实际数据取决于调用参数和当前日期
```
