# ths_daily — 同花顺行业日线行情（THS Daily）

## 数据源名称
- **中文名称**：同花顺行业日线行情
- **英文名称**：THS Daily
- **数据源ID**：DS_TUSHARE_THS_DAILY

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.ths_daily(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，ths_daily 接口提供 同花顺行业日线行情 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.ths_daily(参数1=值1, 参数2=值2)
```
...
| 字段名 | 类型 | 说明 | 数据示例 |
|:------|:----:|:-----|:--------|
| turnover_rate | float | turnover_rate（同花顺） | ? |
| vol | float | vol（同花顺） | ? |
| pct_change | float | pct_change（同花顺） | ? |
| change | float | change（同花顺） | ? |
| avg_price | float | avg_price（同花顺） | ? |
| pre_close | float | pre_close（同花顺） | ? |
| close | float | close（同花顺） | ? |
| low | float | low（同花顺） | ? |
| high | float | high（同花顺） | ? |
| open | float | open（同花顺） | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.ths_daily(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.ths_daily(...) 返回的 DataFrame
# 示例：columns = ['turnover_rate', 'vol', 'pct_change', 'change', 'avg_price'] (前5列)
# 实际数据取决于调用参数和当前日期
```
