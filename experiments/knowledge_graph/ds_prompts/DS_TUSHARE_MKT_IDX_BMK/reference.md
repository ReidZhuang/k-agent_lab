# mkt_idx_bmk — 市场指数基准（Market Index BMK）

## 数据源名称
- **中文名称**：市场指数基准
- **英文名称**：Market Index BMK
- **数据源ID**：DS_TUSHARE_MKT_IDX_BMK

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.mkt_idx_bmk(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，mkt_idx_bmk 接口提供 市场指数基准 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.mkt_idx_bmk(参数1=值1, 参数2=值2)
```
...
| 字段名 | 类型 | 说明 | 数据示例 |
|:------|:----:|:-----|:--------|
| idx_type | str | 指数类型 策略类指数；规模类指数；主题类指数；综合类指数；行业类指数；风格类指数 | ? |
| bmk_src | str | 指数编制机构 | ? |
| bmk_type | str | 基准类型 策略、宽基、行业主题 | ? |
| bmk_level | str | 基准库分层 一类库、二类库 | ? |
| fullname | str | 指数名称 | ? |
| name | str | 指数简称 | ? |
| symbol | str | 代码 | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.mkt_idx_bmk(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.mkt_idx_bmk(...) 返回的 DataFrame
# 示例：columns = ['idx_type', 'bmk_src', 'bmk_type', 'bmk_level', 'fullname'] (前5列)
# 实际数据取决于调用参数和当前日期
```
