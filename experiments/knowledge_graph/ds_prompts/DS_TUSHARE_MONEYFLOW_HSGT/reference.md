# moneyflow_hsgt — 沪深港通资金流向（汇总）（Money Flow HSGT）

## 数据源名称
- **中文名称**：沪深港通资金流向（汇总）
- **英文名称**：Money Flow HSGT
- **数据源ID**：DS_TUSHARE_MONEYFLOW_HSGT
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，moneyflow_hsgt 接口提供 沪深港通资金流向（汇总） 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
# 示例：columns = ['south_money', 'north_money', 'sgt', 'hgt', 'ggt_sz'] (前5列)
# 实际数据取决于调用参数和当前日期
```
