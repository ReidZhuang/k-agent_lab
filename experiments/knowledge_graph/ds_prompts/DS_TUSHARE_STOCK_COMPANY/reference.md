# stock_company — 上市公司基本信息（Stock Company）

## 数据源名称
- **中文名称**：上市公司基本信息
- **英文名称**：Stock Company
- **数据源ID**：DS_TUSHARE_STOCK_COMPANY

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.stock_company(ts_code, start_date, end_date)`
- **SDK**：`import tushare as ts; pro = ts.pro_api()`
- **认证方式**：需设置 `TUSHARE_TOKEN` 环境变量（`ts.set_token(os.getenv('TUSHARE_TOKEN'))`）

## 数据内容描述
上市公司详细资料，包括注册地、法人、经营范围、简介等

## 数据内容覆盖业务描述
公司基本面分析、尽调

## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，stock_company 接口提供 上市公司基本信息 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.stock_company(参数1=值1, 参数2=值2)
```
...
| email | float | email | ? |
| introduction | float | introduction | ? |
| city | float | city | ? |
| province | float | province | ? |
| com_id | float | com_id | ? |
| com_name | float | com_name | ? |
| employees | int | 公司员工总数 | ? |
| chairman | string | 公司法人代表 | ? |
| office | string | 办公地址 | ? |
| main_business | string | 主要业务及产品 | ? |
| reg_capital | float | 注册资本（万元） | ? |
| secretary | string | 董事会秘书 | ? |
| business_scope | string | 经营范围描述 | ? |
| website | string | 公司官网URL | ? |
| setup_date | date | 公司注册成立日期 | ? |
| manager | string | 公司总经理 | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.stock_company(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.stock_company(...) 返回的 DataFrame
# 示例：columns = ['exchange', 'email', 'introduction', 'city', 'province'] (前5列)
# 实际数据取决于调用参数和当前日期
```
