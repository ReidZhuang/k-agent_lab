# moneyflow_hsgt — 沪深港通资金流向（HSGT Money Flow）

## 数据源名称
- **中文名称**：沪深港通资金流向
- **英文名称**：HSGT Money Flow
- **数据源ID**：DS_TUSHARE_HSGT

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.moneyflow_hsgt(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，moneyflow_hsgt 接口提供 沪深港通资金流向 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.moneyflow_hsgt(参数1=值1, 参数2=值2)
```
...
| 字段名 | 类型 | 说明 | 数据示例 |
|:------|:----:|:-----|:--------|
| ggt_sz | float | 港股通（深圳）资金 | ? |
| north_money | float | 北向资金净流入（百万元） | ? |
| hgt | float | 沪股通净流入（百万元） | ? |
| trade_date | date | 交易日期 | ? |
| sgt | float | 深股通净流入（百万元） | ? |
| ggt_ss | float | 港股通（上海）资金 | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.moneyflow_hsgt(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.moneyflow_hsgt(...) 返回的 DataFrame
# 示例：columns = ['ggt_sz', 'north_money', 'hgt', 'trade_date', 'sgt'] (前5列)
# 实际数据取决于调用参数和当前日期
```
