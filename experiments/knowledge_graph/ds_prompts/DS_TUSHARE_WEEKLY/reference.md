# weekly — A股周线行情（Weekly Line）

## 数据源名称
- **中文名称**：A股周线行情
- **英文名称**：Weekly Line
- **数据源ID**：DS_TUSHARE_WEEKLY

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.weekly(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，weekly 接口提供 A股周线行情 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.weekly(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.weekly(...) 返回的 DataFrame
