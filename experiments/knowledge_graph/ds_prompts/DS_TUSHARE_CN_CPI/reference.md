...
- **中文名称**：CPI数据
- **英文名称**：CN CPI
- **数据源ID**：DS_TUSHARE_CN_CPI

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.cn_cpi(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，cn_cpi 接口提供 CPI数据 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
| 字段名 | 类型 | 说明 | 数据示例 |
|:------|:----:|:-----|:--------|
| cnt_accu | float | cnt_accu | ? |
| cnt_mom | float | cnt_mom | ? |
| cnt_yoy | float | cnt_yoy | ? |
| cnt_val | float | cnt_val | ? |
| town_accu | float | town_accu | ? |
| town_mom | float | town_mom | ? |
| town_yoy | float | town_yoy | ? |
| town_val | float | town_val | ? |
| nt_accu | float | nt_accu | ? |
| nt_mom | float | nt_mom | ? |
| nt_yoy | float | nt_yoy | ? |
| nt_val | float | nt_val | ? |
| month | float | month | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.cn_cpi(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.cn_cpi(...) 返回的 DataFrame
# 示例：columns = ['cnt_accu', 'cnt_mom', 'cnt_yoy', 'cnt_val', 'town_accu'] (前5列)
# 实际数据取决于调用参数和当前日期
```
