# index_member — 指数成分（Index Member）

## 数据源名称
- **中文名称**：指数成分
- **英文名称**：Index Member
- **数据源ID**：DS_TUSHARE_INDEX_MEMBER

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.index_member(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，index_member 接口提供 指数成分 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.index_member(参数1=值1, 参数2=值2)
```
...
| 字段名 | 类型 | 说明 | 数据示例 |
|:------|:----:|:-----|:--------|
| in_date | date | 股票纳入行业日期 | ? |
| out_date | date | 股票剔除行业日期 | ? |
| is_new | string | 是否为最新成分（Y/N） | ? |
| con_name | string | 行业成分股名称 | ? |
| con_code | string | 行业成分股TS代码 | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.index_member(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.index_member(...) 返回的 DataFrame
# 示例：columns = ['in_date', 'out_date', 'is_new', 'con_name', 'con_code'] (前5列)
# 实际数据取决于调用参数和当前日期
```
