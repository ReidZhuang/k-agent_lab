# bak_basic — 备用行情基础信息（Backup Basic）

## 数据源名称
- **中文名称**：备用行情基础信息
- **英文名称**：Backup Basic
- **数据源ID**：DS_TUSHARE_BAK_BASIC

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.bak_basic(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，bak_basic 接口提供 备用行情基础信息 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.bak_basic(参数1=值1, 参数2=值2)
```
...
| 字段名 | 类型 | 说明 | 数据示例 |
|:------|:----:|:-----|:--------|
| holder_num | float | holder_num | ? |
| npr | float | npr | ? |
| gpr | float | gpr | ? |
| profit_yoy | float | profit_yoy | ? |
| rev_yoy | float | rev_yoy | ? |
| per_undp | float | per_undp | ? |
| undp | float | undp | ? |
| list_date | float | list_date | ? |
| pb | float | pb | ? |
| bvps | float | bvps | ? |
| eps | float | eps | ? |
| reserved_pershare | float | reserved_pershare | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.bak_basic(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.bak_basic(...) 返回的 DataFrame
# 示例：columns = ['holder_num', 'npr', 'gpr', 'profit_yoy', 'rev_yoy'] (前5列)
# 实际数据取决于调用参数和当前日期
```
