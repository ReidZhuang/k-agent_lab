# fut_holding — 期货持仓（Futures Holding）

## 数据源名称
- **中文名称**：期货持仓
- **英文名称**：Futures Holding
- **数据源ID**：DS_TUSHARE_FUT_HOLDING

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.fut_holding(trade_date, symbol)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，fut_holding 接口提供 期货持仓 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.fut_holding(参数1=值1, 参数2=值2)
```
...
| vol_chg | int | 成交量变化 | ? |
| vol | int | 期货公司成交量 | ? |
| short_chg | int | 空头持仓量变化 | ? |
| short_hld | int | 空头持仓量 | ? |
| broker | string | 期货公司会员简称 | ? |
| long_chg | int | 多头持仓量变化 | ? |
| long_hld | int | 多头持仓量 | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.fut_holding(ts_code='000001.SZ')
print(df.head(10))
```

## 调用返回值样例
```
# pro.fut_holding(...) 返回的 DataFrame
# 示例：columns = ['vol_chg', 'vol', 'short_chg', 'short_hld', 'broker'] (前5列)
# 实际数据取决于调用参数和当前日期
```
