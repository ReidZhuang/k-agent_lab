# stk_alert — 个股异动（Stock Alert）

## 数据源名称
- **中文名称**：个股异动
- **英文名称**：Stock Alert
- **数据源ID**：DS_TUSHARE_STK_ALERT

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.stk_alert(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，stk_alert 接口提供 个股异动 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.stk_alert(参数1=值1, 参数2=值2)
```
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.stk_alert(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.stk_alert(...) 返回的 DataFrame
# 示例：columns = ['type', 'name'] (前5列)
# 实际数据取决于调用参数和当前日期
```
