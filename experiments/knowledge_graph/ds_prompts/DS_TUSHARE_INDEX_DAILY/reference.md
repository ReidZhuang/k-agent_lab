# index_daily — 指数日线行情（Index Daily）

## 数据源名称
- **中文名称**：指数日线行情
- **英文名称**：Index Daily
- **数据源ID**：DS_TUSHARE_INDEX_DAILY

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.index_daily(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，index_daily 接口提供 指数日线行情 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.index_daily(参数1=值1, 参数2=值2)
```
...
|:------|:----:|:-----|:--------|
| swing | float | 指数当日振幅 | ? |
| amount | float | 指数成交额（千元） | ? |
| vol | float | 指数成交量（手） | ? |
| pre_close | float | 前一交易日收盘点位 | ? |
| open | float | 当日开盘点位 | ? |
| low | float | 当日最低点位 | ? |
| high | float | 当日最高点位 | ? |
| change | float | 指数涨跌点数 | ? |
| pct_chg | float | 指数涨跌幅百分比 | ? |
| close | float | 指数收盘点位 | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.index_daily(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.index_daily(...) 返回的 DataFrame
# 示例：columns = ['swing', 'amount', 'vol', 'pre_close', 'open'] (前5列)
# 实际数据取决于调用参数和当前日期
```
