# fund_nav — 基金净值（Fund NAV）

## 数据源名称
- **中文名称**：基金净值
- **英文名称**：Fund NAV
- **数据源ID**：DS_TUSHARE_FUND_NAV

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.fund_nav(ts_code, nav_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，fund_nav 接口提供 基金净值 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
## 调用返回值样例
```
# pro.fund_nav(...) 返回的 DataFrame
# 示例：columns = ['accum_nav', 'unit_nav'] (前5列)
# 实际数据取决于调用参数和当前日期
```
