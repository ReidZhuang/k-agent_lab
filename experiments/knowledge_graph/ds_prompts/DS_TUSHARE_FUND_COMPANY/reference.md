# fund_company — 基金公司（Fund Company）

## 数据源名称
- **中文名称**：基金公司
- **英文名称**：Fund Company
- **数据源ID**：DS_TUSHARE_FUND_COMPANY

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.fund_company(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，fund_company 接口提供 基金公司 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.fund_company(参数1=值1, 参数2=值2)
```
...
| 字段名 | 类型 | 说明 | 数据示例 |
|:------|:----:|:-----|:--------|
| credit_code | float | credit_code | ? |
| org_code | float | org_code | ? |
| main_business | float | main_business | ? |
| employees | float | employees | ? |
| setup_date | float | setup_date | ? |
| reg_capital | float | reg_capital | ? |
| manager | float | manager | ? |
| chairman | float | chairman | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.fund_company(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.fund_company(...) 返回的 DataFrame
# 示例：columns = ['credit_code', 'org_code', 'main_business', 'employees', 'setup_date'] (前5列)
# 实际数据取决于调用参数和当前日期
```
