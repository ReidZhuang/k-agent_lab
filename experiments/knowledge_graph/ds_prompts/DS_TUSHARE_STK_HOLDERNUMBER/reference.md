# stk_holdernumber — 股东人数（Holder Number）

## 数据源名称
- **中文名称**：股东人数
- **英文名称**：Holder Number
- **数据源ID**：DS_TUSHARE_STK_HOLDERNUMBER

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.stk_holdernumber(ts_code)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，stk_holdernumber 接口提供 股东人数 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.stk_holdernumber(参数1=值1, 参数2=值2)
```
...
## 调用返回值样例
```
# pro.stk_holdernumber(...) 返回的 DataFrame
# 示例：columns = ['holder_num'] (前5列)
# 实际数据取决于调用参数和当前日期
```
...
7. **特别提醒**：只传 ts_code，不传日期参数。日期参数会导致数据为空。
