...
- **中文名称**：分红送股
- **英文名称**：Dividend
- **数据源ID**：DS_TUSHARE_DIVIDEND

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.dividend(ts_code)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，dividend 接口提供 分红送股 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
| 字段名 | 类型 | 说明 | 数据示例 |
|:------|:----:|:-----|:--------|
| imp_ann_date | float | imp_ann_date | ? |
| div_listdate | float | div_listdate | ? |
| pay_date | date | 现金红利派发日期 | ? |
| record_date | date | 股权登记日期 | ? |
| ann_date | date | 分红预案公告日期 | ? |
| cash_div | float | 每股税后现金分红 | ? |
| end_date | date | 分红对应的财务年度 | ? |
| stk_bo_rate | float | 每股送股比例 | ? |
| ex_date | date | 除权除息日期 | ? |
| cash_div_tax | float | 每股税前现金分红 | ? |
| stk_div | float | 每股送股+转增合计 | ? |
| stk_co_rate | float | 每股转增比例 | ? |
| div_proc | string | 实施进度（预案/实施/取消） | ? |
...
## 调用返回值样例
```
# pro.dividend(...) 返回的 DataFrame
# 示例：columns = ['imp_ann_date', 'div_listdate', 'pay_date', 'record_date', 'ann_date'] (前5列)
# 实际数据取决于调用参数和当前日期
```
