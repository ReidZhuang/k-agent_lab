# ggt_top10 — 港股通十大成交股（GGT Top10）

## 数据源名称
- **中文名称**：港股通十大成交股
- **英文名称**：GGT Top10
- **数据源ID**：DS_TUSHARE_GGT_TOP10

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.ggt_top10(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，ggt_top10 接口提供 港股通十大成交股 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.ggt_top10(参数1=值1, 参数2=值2)
```
...
| 字段名 | 类型 | 说明 | 数据示例 |
|:------|:----:|:-----|:--------|
| sz_sell | float | sz_sell | ? |
| sz_buy | float | sz_buy | ? |
| sz_net_amount | float | sz_net_amount | ? |
| sz_amount | float | sz_amount | ? |
| sh_sell | float | sh_sell | ? |
| sh_buy | float | sh_buy | ? |
| sh_net_amount | float | sh_net_amount | ? |
| sh_amount | float | sh_amount | ? |
| net_amount | float | net_amount | ? |
| amount | float | amount | ? |
| market_type | float | market_type | ? |
| rank | float | rank | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.ggt_top10(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.ggt_top10(...) 返回的 DataFrame
# 示例：columns = ['sz_sell', 'sz_buy', 'sz_net_amount', 'sz_amount', 'sh_sell'] (前5列)
# 实际数据取决于调用参数和当前日期
```
