...
- **中文名称**：业绩快报
- **英文名称**：Express
- **数据源ID**：DS_TUSHARE_EXPRESS

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.express(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，express 接口提供 业绩快报 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
| 字段名 | 类型 | 说明 | 数据示例 |
|:------|:----:|:-----|:--------|
| perf_summary | str | perf_summary | ? |
| bps | float | bps | ? |
| yoy_net_profit | float | yoy_net_profit | ? |
| diluted_roe | float | diluted_roe | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.express(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.express(...) 返回的 DataFrame
# 示例：columns = ['perf_summary', 'bps', 'yoy_net_profit', 'diluted_roe', 'diluted_eps'] (前5列)
# 实际数据取决于调用参数和当前日期
```
