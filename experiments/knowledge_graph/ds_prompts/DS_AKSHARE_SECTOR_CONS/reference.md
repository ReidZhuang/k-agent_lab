# DS_AKSHARE_SECTOR_CONS — 东方财富板块成分股（Sector Constituents）

## 数据源名称
- **中文名称**：东方财富板块成分股
- **英文名称**：Sector Constituents
- **数据源ID**：DS_AKSHARE_SECTOR_CONS

## 接口
- **类型**：akshare SDK（B类）
- **函数签名**：`ak.stock_board_industry_cons_em(symbol)`
...
```
# 返回值格式
# ak.stock_board_industry_cons_em(symbol) 的返回值
# 实际数据需运行时获取
```
