# moneyflow_cnt_ths — THS板块资金流统计（THS MoneyFlow Count）

## 数据源名称
- **中文名称**：THS板块资金流统计
- **英文名称**：THS MoneyFlow Count
- **数据源ID**：DS_TUSHARE_MONEYFLOW_CNT_THS

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.moneyflow_cnt_ths(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，moneyflow_cnt_ths 接口提供 THS板块资金流统计 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.moneyflow_cnt_ths(参数1=值1, 参数2=值2)
```
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.moneyflow_cnt_ths(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.moneyflow_cnt_ths(...) 返回的 DataFrame
# 示例：columns = ['net_amount', 'net_sell_amount', 'net_buy_amount', 'pct_change_stock', 'company_num'] (前5列)
# 实际数据取决于调用参数和当前日期
```
