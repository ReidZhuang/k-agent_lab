# stk_surv — 股票存活状态（Stock Survival）

## 数据源名称
- **中文名称**：股票存活状态
- **英文名称**：Stock Survival
- **数据源ID**：DS_TUSHARE_STK_SURV

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.stk_surv(ts_code)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，stk_surv 接口提供 股票存活状态 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.stk_surv(ts_code='000001.SZ')
print(df.head(10))
```

## 调用返回值样例
```
# pro.stk_surv(...) 返回的 DataFrame
# 示例：columns = ['comp_rece', 'org_type', 'rece_org', 'rece_mode', 'rece_place'] (前5列)
# 实际数据取决于调用参数和当前日期
```
...
7. **特别提醒**：只传 ts_code，返回 DataFrame
