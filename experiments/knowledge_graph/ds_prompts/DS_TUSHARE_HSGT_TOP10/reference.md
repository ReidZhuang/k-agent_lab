# hsgt_top10 — 沪深港通十大成交股（HSGT Top10）

## 数据源名称
- **中文名称**：沪深港通十大成交股
- **英文名称**：HSGT Top10
- **数据源ID**：DS_TUSHARE_HSGT_TOP10

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.hsgt_top10(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，hsgt_top10 接口提供 沪深港通十大成交股 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.hsgt_top10(参数1=值1, 参数2=值2)
```
...
| close | float | close | ? |
| name | float | name | ? |
| net_amount | float | 十大成交股净买入金额 | ? |
| ts_code | string | 沪深股通十大成交股列表 | ? |
| amount | float | 十大成交股成交金额 | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.hsgt_top10(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.hsgt_top10(...) 返回的 DataFrame
# 示例：columns = ['sell', 'buy', 'market_type', 'rank', 'change'] (前5列)
# 实际数据取决于调用参数和当前日期
```
