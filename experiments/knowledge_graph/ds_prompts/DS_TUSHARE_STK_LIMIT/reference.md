# stk_limit — 个股涨跌停价格（Stock Limit）

## 数据源名称
- **中文名称**：个股涨跌停价格
- **英文名称**：Stock Limit
- **数据源ID**：DS_TUSHARE_STK_LIMIT

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.stk_limit(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，stk_limit 接口提供 个股涨跌停价格 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.stk_limit(参数1=值1, 参数2=值2)
```
...
| 字段名 | 类型 | 说明 | 数据示例 |
|:------|:----:|:-----|:--------|
| pre_close | float | 昨日收盘价（除权价） | ? |
| up_limit | float | 当日涨停价格 | ? |
| down_limit | float | 当日跌停价格 | ? |
| trade_date | date | 交易日期 | ? |
| stock_code | string | 股票TS代码 | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.stk_limit(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.stk_limit(...) 返回的 DataFrame
# 示例：columns = ['pre_close', 'up_limit', 'down_limit', 'trade_date', 'stock_code'] (前5列)
# 实际数据取决于调用参数和当前日期
```
