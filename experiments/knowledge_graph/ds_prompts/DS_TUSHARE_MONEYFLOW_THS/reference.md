# moneyflow_ths — 同花顺资金流向（THS Money Flow）

## 数据源名称
- **中文名称**：同花顺资金流向
- **英文名称**：THS Money Flow
- **数据源ID**：DS_TUSHARE_MONEYFLOW_THS

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.moneyflow_ths(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，moneyflow_ths 接口提供 同花顺资金流向 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.moneyflow_ths(参数1=值1, 参数2=值2)
```
...
| net_d5_amount | float | net_d5_amount | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.moneyflow_ths(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.moneyflow_ths(...) 返回的 DataFrame
# 示例：columns = ['buy_sm_amount_rate', 'buy_md_amount_rate', 'buy_lg_amount_rate', 'net_d5_amount', 'net_amount'] (前5列)
# 实际数据取决于调用参数和当前日期
```
