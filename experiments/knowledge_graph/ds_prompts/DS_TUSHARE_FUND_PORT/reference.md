# fund_portfolio — 基金持仓（Fund Portfolio）

## 数据源名称
- **中文名称**：基金持仓
- **英文名称**：Fund Portfolio
- **数据源ID**：DS_TUSHARE_FUND_PORT

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.fund_portfolio(ts_code)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，fund_portfolio 接口提供 基金持仓 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.fund_portfolio(参数1=值1, 参数2=值2)
```
...
| mkv | float | mkv | ? |
| amount | float | amount | ? |
| stk_mkv_ratio | float | stk_mkv_ratio | ? |
| stk_float_ratio | float | stk_float_ratio | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.fund_portfolio(ts_code='000001.SZ')
print(df.head(10))
```

## 调用返回值样例
```
# pro.fund_portfolio(...) 返回的 DataFrame
# 示例：columns = ['ts_code', 'ann_date', 'end_date', 'symbol', 'mkv'] (前5列)
# 实际数据取决于调用参数和当前日期
```
