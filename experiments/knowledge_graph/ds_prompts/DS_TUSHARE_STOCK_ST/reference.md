# stock_st — ST股票列表（Stock ST）

## 数据源名称
- **中文名称**：ST股票列表
- **英文名称**：Stock ST
- **数据源ID**：DS_TUSHARE_STOCK_ST

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.stock_st(trade_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，stock_st 接口提供 ST股票列表 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.stock_st(ts_code='000001.SZ')
print(df.head(10))
```

## 调用返回值样例
```
# pro.stock_st(...) 返回的 DataFrame
# 示例：columns = ['type_name', 'type', 'name'] (前5列)
# 实际数据取决于调用参数和当前日期
```
