# fina_mainbz — 主营业务构成（Main Business）

## 数据源名称
- **中文名称**：主营业务构成
- **英文名称**：Main Business
- **数据源ID**：DS_TUSHARE_FINA_MAINBZ

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.fina_mainbz(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，fina_mainbz 接口提供 主营业务构成 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.fina_mainbz(参数1=值1, 参数2=值2)
```
...
| bz_cost | float | bz_cost | ? |
| bz_profit | float | bz_profit | ? |
| bz_sales | float | bz_sales | ? |
| bz_code | str | bz_code | ? |
| bz_item | str | bz_item | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.fina_mainbz(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.fina_mainbz(...) 返回的 DataFrame
# 示例：columns = ['curr_type', 'bz_cost', 'bz_profit', 'bz_sales', 'bz_code'] (前5列)
# 实际数据取决于调用参数和当前日期
```
