# moneyflow_ind_ths — THS行业资金流向（THS Industry MoneyFlow）

## 数据源名称
- **中文名称**：THS行业资金流向
- **英文名称**：THS Industry MoneyFlow
- **数据源ID**：DS_TUSHARE_MONEYFLOW_IND_THS

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.moneyflow_ind_ths(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，moneyflow_ind_ths 接口提供 THS行业资金流向 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.moneyflow_ind_ths(参数1=值1, 参数2=值2)
```
...
| net_sell_amount | float | net_sell_amount | ? |
| net_buy_amount | float | net_buy_amount | ? |
| close_price | float | close_price | ? |
| pct_change_stock | float | pct_change_stock | ? |
| company_num | float | company_num | ? |
| pct_change | float | pct_change | ? |
| close | float | close | ? |
| lead_stock | float | lead_stock | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.moneyflow_ind_ths(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.moneyflow_ind_ths(...) 返回的 DataFrame
# 示例：columns = ['net_amount', 'net_sell_amount', 'net_buy_amount', 'close_price', 'pct_change_stock'] (前5列)
# 实际数据取决于调用参数和当前日期
```
