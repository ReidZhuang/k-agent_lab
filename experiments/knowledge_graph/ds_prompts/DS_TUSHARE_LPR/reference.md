# shibor_lpr — LPR利率（LPR）

## 数据源名称
- **中文名称**：LPR利率
- **英文名称**：LPR
- **数据源ID**：DS_TUSHARE_LPR
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，shibor_lpr 接口提供 LPR利率 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
