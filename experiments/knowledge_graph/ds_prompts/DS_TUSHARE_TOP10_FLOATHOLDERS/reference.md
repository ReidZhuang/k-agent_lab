# top10_floatholders — 前十大流通股东（Top10 Float Holders）

## 数据源名称
- **中文名称**：前十大流通股东
- **英文名称**：Top10 Float Holders
- **数据源ID**：DS_TUSHARE_TOP10_FLOATHOLDERS

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.top10_floatholders(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，top10_floatholders 接口提供 前十大流通股东 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.top10_floatholders(参数1=值1, 参数2=值2)
```
...
| 字段名 | 类型 | 说明 | 数据示例 |
|:------|:----:|:-----|:--------|
| holder_type | float | holder_type | ? |
| hold_change | float | hold_change | ? |
| hold_float_ratio | float | hold_float_ratio | ? |
| hold_ratio | float | hold_ratio | ? |
| hold_amount | float | hold_amount | ? |
| holder_name | float | holder_name | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.top10_floatholders(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.top10_floatholders(...) 返回的 DataFrame
# 示例：columns = ['holder_type', 'hold_change', 'hold_float_ratio', 'hold_ratio', 'hold_amount'] (前5列)
# 实际数据取决于调用参数和当前日期
```
