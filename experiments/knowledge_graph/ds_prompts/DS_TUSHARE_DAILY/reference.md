# daily — A股日线行情（Daily Line）

## 数据源名称
- **中文名称**：A股日线行情
- **英文名称**：Daily Line
- **数据源ID**：DS_TUSHARE_DAILY

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.daily(ts_code, start_date, end_date)`
- **SDK**：`import tushare as ts; pro = ts.pro_api()`
- **认证方式**：需设置 `TUSHARE_TOKEN` 环境变量（`ts.set_token(os.getenv('TUSHARE_TOKEN'))`）

## 数据内容描述
A股每日交易行情数据，包括开盘价、收盘价、最高价、最低价、成交量、成交额等

## 数据内容覆盖业务描述
个股日频行情分析、技术指标计算、回测

## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，daily 接口提供 A股日线行情 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.daily(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.daily(...) 返回的 DataFrame
# 示例：columns = ['open', 'vol', 'trade_date', 'close', 'pct_chg'] (前5列)
# 实际数据取决于调用参数和当前日期
```
