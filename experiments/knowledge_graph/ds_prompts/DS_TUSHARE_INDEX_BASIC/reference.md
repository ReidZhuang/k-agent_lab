# index_basic — 指数基本信息（Index Basic）

## 数据源名称
- **中文名称**：指数基本信息
- **英文名称**：Index Basic
- **数据源ID**：DS_TUSHARE_INDEX_BASIC

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.index_basic(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，index_basic 接口提供 指数基本信息 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.index_basic(参数1=值1, 参数2=值2)
```
...
| base_point | float | base_point | ? |
| base_date | float | base_date | ? |
| category | float | category | ? |
| publisher | float | publisher | ? |
| market | float | market | ? |
| ts_code | string | 指数TS代码 | ? |
| name | string | 指数的中文名称 | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.index_basic(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.index_basic(...) 返回的 DataFrame
# 示例：columns = ['list_date', 'base_point', 'base_date', 'category', 'publisher'] (前5列)
# 实际数据取决于调用参数和当前日期
```
