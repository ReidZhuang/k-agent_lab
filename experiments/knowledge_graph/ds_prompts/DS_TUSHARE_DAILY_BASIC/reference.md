# daily_basic — 每日指标（Daily Basic）

## 数据源名称
- **中文名称**：每日指标
- **英文名称**：Daily Basic
- **数据源ID**：DS_TUSHARE_DAILY_BASIC

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.daily_basic(ts_code, start_date, end_date)`
- **SDK**：`import tushare as ts; pro = ts.pro_api()`
- **认证方式**：需设置 `TUSHARE_TOKEN` 环境变量（`ts.set_token(os.getenv('TUSHARE_TOKEN'))`）

## 数据内容描述
每日涨跌幅、换手率、量比、市盈率、市净率等基础指标

## 数据内容覆盖业务描述
股票每日估值指标、市场情绪分析

## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，daily_basic 接口提供 每日指标 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.daily_basic(参数1=值1, 参数2=值2)
```
...
| ps_ttm | float | 滚动市销率 | ? |
| pb | float | 市净率 | ? |
| 涨跌停状态 | int | 0平/1涨(非停)/2涨停(非一字)/3一字涨停/4跌(非停)/5跌停(非一字)/6一字跌停 | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.daily_basic(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.daily_basic(...) 返回的 DataFrame
# 示例：columns = ['circ_mv', 'total_mv', 'free_share', 'float_share', 'total_share'] (前5列)
# 实际数据取决于调用参数和当前日期
```
