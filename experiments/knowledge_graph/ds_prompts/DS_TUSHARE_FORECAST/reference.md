...
- **中文名称**：业绩预告
- **英文名称**：Forecast
- **数据源ID**：DS_TUSHARE_FORECAST

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.forecast(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，forecast 接口提供 业绩预告 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
| summary | str | summary | ? |
| last_parent_net | float | last_parent_net | ? |
| net_profit_max | float | net_profit_max | ? |
| net_profit_min | float | net_profit_min | ? |
| p_change_max | float | p_change_max | ? |
| p_change_min | float | p_change_min | ? |
| type | str | type | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.forecast(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.forecast(...) 返回的 DataFrame
# 示例：columns = ['change_reason', 'summary', 'last_parent_net', 'net_profit_max', 'net_profit_min'] (前5列)
# 实际数据取决于调用参数和当前日期
```
