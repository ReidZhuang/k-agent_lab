# ci_index_member — CI指数成分（CI Index Member）

## 数据源名称
- **中文名称**：CI指数成分
- **英文名称**：CI Index Member
- **数据源ID**：DS_TUSHARE_CI_INDEX_MEMBER

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.ci_index_member(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，ci_index_member 接口提供 CI指数成分 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.ci_index_member(参数1=值1, 参数2=值2)
```
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.ci_index_member(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.ci_index_member(...) 返回的 DataFrame
