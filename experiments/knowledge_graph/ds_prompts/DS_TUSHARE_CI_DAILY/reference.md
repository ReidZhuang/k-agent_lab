# ci_daily — CI行业日线（CI Daily）

## 数据源名称
- **中文名称**：CI行业日线
- **英文名称**：CI Daily
- **数据源ID**：DS_TUSHARE_CI_DAILY

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.ci_daily(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，ci_daily 接口提供 CI行业日线 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
| 字段名 | 类型 | 说明 | 数据示例 |
|:------|:----:|:-----|:--------|
| amount | float | amount（中信） | ? |
| vol | float | vol（中信） | ? |
| pct_change | float | pct_change（中信） | ? |
| change | float | change（中信） | ? |
| pre_close | float | pre_close（中信） | ? |
| close | float | close（中信） | ? |
| high | float | high（中信） | ? |
| low | float | low（中信） | ? |
| open | float | open（中信） | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.ci_daily(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.ci_daily(...) 返回的 DataFrame
# 示例：columns = ['amount', 'vol', 'pct_change', 'change', 'pre_close'] (前5列)
# 实际数据取决于调用参数和当前日期
```
