# bc_otcqt — 场外行情（OTC Quote）

## 数据源名称
- **中文名称**：场外行情
- **英文名称**：OTC Quote
- **数据源ID**：DS_TUSHARE_BC_OTCQT

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.bc_otcqt(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，bc_otcqt 接口提供 场外行情 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
| BID_BANK | float | BID_BANK | ? |
| BOND_TYPE | float | BOND_TYPE | ? |
| QT_TIME | float | QT_TIME | ? |
| CREATE_BY | float | CREATE_BY | ? |
| COUPON_RATE | float | COUPON_RATE | ? |
| REMAIN_MATURITY | float | REMAIN_MATURITY | ? |
| NAME | float | NAME | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.bc_otcqt(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.bc_otcqt(...) 返回的 DataFrame
# 示例：columns = ['BUY_YIELD', 'SELL_PRICE', 'BUY_PRICE', 'BID_BANK', 'BOND_TYPE'] (前5列)
# 实际数据取决于调用参数和当前日期
```
