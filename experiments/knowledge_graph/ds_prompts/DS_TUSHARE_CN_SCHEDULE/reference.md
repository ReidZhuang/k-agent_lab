# cn_schedule — A股日历（CN Schedule）

## 数据源名称
- **中文名称**：A股日历
- **英文名称**：CN Schedule
- **数据源ID**：DS_TUSHARE_CN_SCHEDULE

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.cn_schedule(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，cn_schedule 接口提供 A股日历 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.cn_schedule(参数1=值1, 参数2=值2)
```
...
| 字段名 | 类型 | 说明 | 数据示例 |
|:------|:----:|:-----|:--------|
| data_api | str | tushare对应接口 | ? |
| issuing_org | str | 发布单位 | ? |
| title | str | 发布数据 | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.cn_schedule(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.cn_schedule(...) 返回的 DataFrame
# 示例：columns = ['data_api', 'issuing_org', 'title', 'publish_date', 'month'] (前5列)
# 实际数据取决于调用参数和当前日期
```
