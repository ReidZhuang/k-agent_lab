# block_trade — 大宗交易（Block Trade）

## 数据源名称
- **中文名称**：大宗交易
- **英文名称**：Block Trade
- **数据源ID**：DS_TUSHARE_BLOCK_TRADE

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.block_trade(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，block_trade 接口提供 大宗交易 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.block_trade(参数1=值1, 参数2=值2)
```
...
| 字段名 | 类型 | 说明 | 数据示例 |
|:------|:----:|:-----|:--------|
| buyer | string | 大宗交易买方营业部 | ? |
| amount | float | 大宗交易成交金额 | ? |
| price | float | 大宗交易成交价格 | ? |
| seller | string | 大宗交易卖方营业部 | ? |
| vol | float | 大宗交易成交量 | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.block_trade(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.block_trade(...) 返回的 DataFrame
# 示例：columns = ['buyer', 'amount', 'price', 'seller', 'vol'] (前5列)
# 实际数据取决于调用参数和当前日期
```
