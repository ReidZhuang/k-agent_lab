# shibor_quote — Shibor报价（SHIBOR Quote）

## 数据源名称
- **中文名称**：Shibor报价
- **英文名称**：SHIBOR Quote
- **数据源ID**：DS_TUSHARE_SHIBOR_QUOTE

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.shibor_quote(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，shibor_quote 接口提供 Shibor报价 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.shibor_quote(参数1=值1, 参数2=值2)
```
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.shibor_quote(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.shibor_quote(...) 返回的 DataFrame
# 示例：columns = ['1y_a', '1y_b', '9m_a', '9m_b', '6m_a'] (前5列)
# 实际数据取决于调用参数和当前日期
```
