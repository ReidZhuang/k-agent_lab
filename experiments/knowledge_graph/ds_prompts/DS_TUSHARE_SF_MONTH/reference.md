# sf_month — 社会融资月度（Social Financing Monthly）

## 数据源名称
- **中文名称**：社会融资月度
- **英文名称**：Social Financing Monthly
- **数据源ID**：DS_TUSHARE_SF_MONTH

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.sf_month(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，sf_month 接口提供 社会融资月度 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
| 字段名 | 类型 | 说明 | 数据示例 |
|:------|:----:|:-----|:--------|
| stk_endval | float | stk_endval | ? |
| inc_cumval | float | inc_cumval | ? |
| inc_month | float | inc_month | ? |
| month | float | month | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.sf_month(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.sf_month(...) 返回的 DataFrame
# 示例：columns = ['stk_endval', 'inc_cumval', 'inc_month', 'month'] (前5列)
# 实际数据取决于调用参数和当前日期
```
