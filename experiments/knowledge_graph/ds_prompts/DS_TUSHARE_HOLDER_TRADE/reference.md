# stk_holdertrade — 股东交易（Holder Trade）

## 数据源名称
- **中文名称**：股东交易
- **英文名称**：Holder Trade
- **数据源ID**：DS_TUSHARE_HOLDER_TRADE

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.stk_holdertrade(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，stk_holdertrade 接口提供 股东交易 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.stk_holdertrade(参数1=值1, 参数2=值2)
```
...
| change_vol | float | 变动数量（股） | ? |
| avg_price | float | 交易平均价格（元） | ? |
| in_de | string | IN增持/DE减持 | ? |
| ann_date | date | 增减持公告日期 | ? |
| after_share | float | 变动后持股数量（股） | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.stk_holdertrade(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.stk_holdertrade(...) 返回的 DataFrame
# 示例：columns = ['total_share', 'after_ratio', 'change_ratio', 'holder_type', 'holder_name'] (前5列)
# 实际数据取决于调用参数和当前日期
```
