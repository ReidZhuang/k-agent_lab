# stk_rewards — 管理层薪酬（Stock Rewards）

## 数据源名称
- **中文名称**：管理层薪酬
- **英文名称**：Stock Rewards
- **数据源ID**：DS_TUSHARE_STK_REWARDS

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.stk_rewards(ts_code)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，stk_rewards 接口提供 管理层薪酬 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.stk_rewards(参数1=值1, 参数2=值2)
```
...
## 调用返回值样例
```
# pro.stk_rewards(...) 返回的 DataFrame
# 示例：columns = ['hold_vol', 'reward', 'title', 'name'] (前5列)
# 实际数据取决于调用参数和当前日期
```
