# bond_blk_detail — 债券大宗交易明细（Bond Block Detail）

## 数据源名称
- **中文名称**：债券大宗交易明细
- **英文名称**：Bond Block Detail
- **数据源ID**：DS_TUSHARE_BOND_BLK_DETAIL

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.bond_blk_detail(trade_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，bond_blk_detail 接口提供 债券大宗交易明细 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.bond_blk_detail(参数1=值1, 参数2=值2)
```
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.bond_blk_detail(ts_code='000001.SZ')
print(df.head(10))
```

## 调用返回值样例
```
# pro.bond_blk_detail(...) 返回的 DataFrame
# 示例：columns = ['sell_dp', 'buy_dp', 'amount', 'vol', 'price'] (前5列)
# 实际数据取决于调用参数和当前日期
```
