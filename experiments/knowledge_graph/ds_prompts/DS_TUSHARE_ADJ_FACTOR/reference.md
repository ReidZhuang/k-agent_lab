# adj_factor — 复权因子（Adjust Factor）

## 数据源名称
- **中文名称**：复权因子
- **英文名称**：Adjust Factor
- **数据源ID**：DS_TUSHARE_ADJ_FACTOR

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.adj_factor(ts_code, start_date, end_date)`
- **SDK**：`import tushare as ts; pro = ts.pro_api()`
- **认证方式**：需设置 `TUSHARE_TOKEN` 环境变量（`ts.set_token(os.getenv('TUSHARE_TOKEN'))`）

## 数据内容描述
股票复权因子，用于计算前复权和后复权价格

## 数据内容覆盖业务描述
价格复权计算，确保历史价格连续性

## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，adj_factor 接口提供 复权因子 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.adj_factor(参数1=值1, 参数2=值2)
```
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.adj_factor(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.adj_factor(...) 返回的 DataFrame
