# shibor — Shibor利率（SHIBOR）

## 数据源名称
- **中文名称**：Shibor利率
- **英文名称**：SHIBOR
- **数据源ID**：DS_TUSHARE_SHIBOR

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.shibor(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，shibor 接口提供 Shibor利率 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
|:------|:----:|:-----|:--------|
| date | date | 利率日期 | ? |
| 3m | float | Shibor3个月利率 | ? |
| 6m | float | Shibor6个月利率 | ? |
| 1w | float | Shibor1周利率 | ? |
| 1y | float | Shibor1年期利率 | ? |
| 9m | float | Shibor9个月利率 | ? |
| 1m | float | Shibor1个月利率 | ? |
| on | float | Shibor隔夜利率 | ? |
| 2w | float | Shibor2周利率 | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.shibor(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.shibor(...) 返回的 DataFrame
# 示例：columns = ['date', '3m', '6m', '1w', '1y'] (前5列)
# 实际数据取决于调用参数和当前日期
```
