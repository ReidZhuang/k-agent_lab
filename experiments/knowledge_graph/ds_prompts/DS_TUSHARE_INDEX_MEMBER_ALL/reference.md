# index_member_all — 指数成分全量（Index Member All）

## 数据源名称
- **中文名称**：指数成分全量
- **英文名称**：Index Member All
- **数据源ID**：DS_TUSHARE_INDEX_MEMBER_ALL

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.index_member_all(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，index_member_all 接口提供 指数成分全量 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.index_member_all(参数1=值1, 参数2=值2)
```
...
|:------|:----:|:-----|:--------|
| is_new | float | is_new | ? |
| out_date | float | out_date | ? |
| in_date | float | in_date | ? |
| name | float | name | ? |
| l3_name | float | l3_name | ? |
| l3_code | float | l3_code | ? |
| l2_name | float | l2_name | ? |
| l2_code | float | l2_code | ? |
| l1_name | float | l1_name | ? |
| l1_code | float | l1_code | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.index_member_all(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.index_member_all(...) 返回的 DataFrame
# 示例：columns = ['is_new', 'out_date', 'in_date', 'name', 'l3_name'] (前5列)
# 实际数据取决于调用参数和当前日期
```
