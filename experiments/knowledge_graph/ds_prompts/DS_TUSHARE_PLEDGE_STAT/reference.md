# pledge_stat — 股权质押统计（Pledge Stat）

## 数据源名称
- **中文名称**：股权质押统计
- **英文名称**：Pledge Stat
- **数据源ID**：DS_TUSHARE_PLEDGE_STAT

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.pledge_stat(ts_code)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，pledge_stat 接口提供 股权质押统计 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.pledge_stat(参数1=值1, 参数2=值2)
```
...
| rest_pledge | float | rest_pledge | ? |
| unrest_pledge | float | unrest_pledge | ? |
| pledge_count | float | pledge_count | ? |
| pledge_ratio | float | 质押占总股本比例 | ? |
...
## 调用返回值样例
```
# pro.pledge_stat(...) 返回的 DataFrame
# 示例：columns = ['total_share', 'rest_pledge', 'unrest_pledge', 'pledge_count', 'pledge_ratio'] (前5列)
# 实际数据取决于调用参数和当前日期
```
