# hm_list — 港股通名单（HM List）

## 数据源名称
- **中文名称**：港股通名单
- **英文名称**：HM List
- **数据源ID**：DS_TUSHARE_HM_LIST

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.hm_list(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，hm_list 接口提供 港股通名单 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.hm_list(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.hm_list(...) 返回的 DataFrame
# 示例：columns = ['orgs', 'desc', 'name'] (前5列)
# 实际数据取决于调用参数和当前日期
```
