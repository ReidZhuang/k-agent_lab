...
- **函数签名**：`pro.st(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，st 接口提供 ST信息 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
| 字段名 | 类型 | 说明 | 数据示例 |
|:------|:----:|:-----|:--------|
| st_explain | float | st_explain | ? |
| st_reason | float | st_reason | ? |
| st_type | float | st_type | ? |
| imp_date | float | imp_date | ? |
| pub_date | float | pub_date | ? |
| name | float | name | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.st(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.st(...) 返回的 DataFrame
# 示例：columns = ['st_explain', 'st_reason', 'st_type', 'imp_date', 'pub_date'] (前5列)
# 实际数据取决于调用参数和当前日期
```
