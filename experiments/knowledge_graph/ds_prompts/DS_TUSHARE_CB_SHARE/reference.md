# cb_share — 可转债转股（CB Share）

## 数据源名称
- **中文名称**：可转债转股
- **英文名称**：CB Share
- **数据源ID**：DS_TUSHARE_CB_SHARE
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，cb_share 接口提供 可转债转股 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
| acc_convert_ratio | float | acc_convert_ratio | ? |
| acc_convert_vol | float | acc_convert_vol | ? |
| acc_convert_val | float | acc_convert_val | ? |
| convert_ratio | float | convert_ratio | ? |
| convert_vol | float | convert_vol | ? |
| convert_val | float | convert_val | ? |
| convert_price | float | convert_price | ? |
| convert_price_initial | float | convert_price_initial | ? |
| issue_size | float | issue_size | ? |
| publish_date | float | publish_date | ? |
| bond_short_name | float | bond_short_name | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.cb_share(ts_code='000001.SZ')
print(df.head(10))
```

## 调用返回值样例
```
# pro.cb_share(...) 返回的 DataFrame
# 示例：columns = ['total_shares', 'remain_size', 'acc_convert_ratio', 'acc_convert_vol', 'acc_convert_val'] (前5列)
# 实际数据取决于调用参数和当前日期
```
