...
- **函数签名**：`pro.wz_index(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，wz_index 接口提供 温州指数 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.wz_index(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.wz_index(...) 返回的 DataFrame
# 示例：columns = ['温州12月期利率', '温州1月期利率', '温州3月期利率', '温州6月期利率', '温州综合利率'] (前5列)
# 实际数据取决于调用参数和当前日期
```
