...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，libor 接口提供 Libor利率 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.libor(ts_code='000001.SZ')
print(df.head(10))
```

## 调用返回值样例
```
# pro.libor(...) 返回的 DataFrame
# 示例：columns = ['3m', 'on', '6m'] (前5列)
# 实际数据取决于调用参数和当前日期
```
