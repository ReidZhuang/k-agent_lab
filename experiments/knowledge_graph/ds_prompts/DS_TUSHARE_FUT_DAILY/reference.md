# fut_daily — 期货日线行情（Futures Daily）

## 数据源名称
- **中文名称**：期货日线行情
- **英文名称**：Futures Daily
- **数据源ID**：DS_TUSHARE_FUT_DAILY

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.fut_daily(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，fut_daily 接口提供 期货日线行情 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.fut_daily(参数1=值1, 参数2=值2)
```
...
| settle | float | 当日结算价 | ? |
| change2 | float | 结算价-昨结算价 | ? |
| oi_chg | float | 持仓量变化 | ? |
| ts_code | string | 期货合约TS代码 | ? |
| high | float | 当日最高价 | ? |
| open | float | 当日开盘价 | ? |
| change1 | float | 收盘价-昨结算价 | ? |
| close | float | 当日收盘价 | ? |
| pre_close | float | 昨日收盘价 | ? |
| pre_settle | float | 昨日结算价 | ? |
| oi | float | 持仓量（手） | ? |
| low | float | 当日最低价 | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.fut_daily(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.fut_daily(...) 返回的 DataFrame
# 示例：columns = ['vol', 'trade_date', 'amount', 'settle', 'change2'] (前5列)
# 实际数据取决于调用参数和当前日期
```
