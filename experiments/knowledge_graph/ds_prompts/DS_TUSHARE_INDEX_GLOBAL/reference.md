# index_global — 全球指数行情（Index Global）

## 数据源名称
- **中文名称**：全球指数行情
- **英文名称**：Index Global
- **数据源ID**：DS_TUSHARE_INDEX_GLOBAL

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.index_global()`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，index_global 接口提供 全球指数行情 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.index_global(参数1=值1, 参数2=值2)
```
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.index_global(ts_code='000001.SZ')
print(df.head(10))
```

## 调用返回值样例
```
# pro.index_global(...) 返回的 DataFrame
# 示例：columns = ['vol', 'swing', 'pct_chg', 'change', 'pre_close'] (前5列)
# 实际数据取决于调用参数和当前日期
```
...
7. **特别提醒**：不传参数时返回所有全球指数
7. **特别提醒**：ts_code 和日期参数全部可选
