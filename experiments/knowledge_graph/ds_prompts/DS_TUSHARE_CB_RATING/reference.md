# cb_rating — 可转债评级（CB Rating）

## 数据源名称
- **中文名称**：可转债评级
- **英文名称**：CB Rating
- **数据源ID**：DS_TUSHARE_CB_RATING

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.cb_rating(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，cb_rating 接口提供 可转债评级 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.cb_rating(参数1=值1, 参数2=值2)
```
...
|:------|:----:|:-----|:--------|
| rating | str | 信用等级 | ? |
| rating_com_name | str | 评级机构 | ? |
| rating_date | str | 评级日期 | ? |
| rating_outlook | str | 评级展望 | ? |
| rating_type | str | 评级类别 | ? |
| rating_way | str | 评级方式 | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.cb_rating(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.cb_rating(...) 返回的 DataFrame
# 示例：columns = ['rating', 'rating_com_name', 'rating_date', 'rating_outlook', 'rating_type'] (前5列)
# 实际数据取决于调用参数和当前日期
```
