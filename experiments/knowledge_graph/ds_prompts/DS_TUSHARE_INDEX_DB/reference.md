# index_dailybasic — 指数每日指标（Index Daily Basic）

## 数据源名称
- **中文名称**：指数每日指标
- **英文名称**：Index Daily Basic
- **数据源ID**：DS_TUSHARE_INDEX_DB

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.index_dailybasic(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，index_dailybasic 接口提供 指数每日指标 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.index_dailybasic(参数1=值1, 参数2=值2)
```
...
| turnover_rate | float | 指数成分股换手率 | ? |
| pb | float | 指数成分股加权市净率 | ? |
| pe | float | 指数成分股加权市盈率 | ? |
| float_mv | float | 指数成分股流通市值 | ? |
| total_mv | float | 指数成分股总市值 | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.index_dailybasic(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.index_dailybasic(...) 返回的 DataFrame
# 示例：columns = ['pe_ttm', 'turnover_rate_f', 'free_share', 'float_share', 'total_share'] (前5列)
# 实际数据取决于调用参数和当前日期
```
