# dc_concept — DC概念板块（DC Concept）

## 数据源名称
- **中文名称**：DC概念板块
- **英文名称**：DC Concept
- **数据源ID**：DS_TUSHARE_DC_CONCEPT

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.dc_concept(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，dc_concept 接口提供 DC概念板块 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
pro = ts.pro_api()
df = pro.dc_concept(参数1=值1, 参数2=值2)
```
...
| 字段名 | 类型 | 说明 | 数据示例 |
|:------|:----:|:-----|:--------|
| lead_stock_pct_change | str | 领涨股票涨跌幅 | ? |
| lead_stock_code | str | 领涨股票code | ? |
| lead_stock | str | 领涨股票 | ? |
| main_change | str | 主力净流入（元） | ? |
| z_t_num | str | 涨停数量 | ? |
| strength | str | 强度 | ? |
| sort | str | 排名 | ? |
| hot | str | 热度 | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.dc_concept(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.dc_concept(...) 返回的 DataFrame
# 示例：columns = ['lead_stock_pct_change', 'lead_stock_code', 'lead_stock', 'main_change', 'z_t_num'] (前5列)
# 实际数据取决于调用参数和当前日期
```
