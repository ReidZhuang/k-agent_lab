# hk_hold — 港股通持股（HK Hold）

## 数据源名称
- **中文名称**：港股通持股
- **英文名称**：HK Hold
- **数据源ID**：DS_TUSHARE_HK_HOLD

## 接口
- **类型**：Tushare Pro API（A类）
- **函数签名**：`pro.hk_hold(ts_code, start_date, end_date)`
...
## 数据接口背景描述（若有）
Tushare 是国内主流的金融数据平台之一，hk_hold 接口提供 港股通持股 数据。Tushare 的 Pro API 基于 pandas DataFrame 返回，使用简单。需在 [tushare.pro](https://tushare.pro) 注册获取 token。
...
| name | float | name | ? |
| code | float | code | ? |
| vol | float | 北向资金持股数量（股） | ? |
| ratio | float | 持股占已发行股份比例 | ? |
...
ts.set_token(os.getenv('TUSHARE_TOKEN'))
pro = ts.pro_api()
df = pro.hk_hold(ts_code='000001.SZ', start_date='20260701', end_date='20260715')
print(df.head(10))
```

## 调用返回值样例
```
# pro.hk_hold(...) 返回的 DataFrame
# 示例：columns = ['exchange', 'name', 'code', 'vol', 'ratio'] (前5列)
# 实际数据取决于调用参数和当前日期
```
