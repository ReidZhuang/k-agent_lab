...
| **行业类关键词（已打标）** | **1,413** | 已设置 `match_class` + `grade` 属性 |
|  MG1 / MG2 / MG3 | 179 / 1,102 / 21 | 有匹配意义，按泛化程度分三级 |
|  NM1 / NM2 / NM3 | 48 / 15 / 48 | 无匹配意义，不参与匹配 |
| 匹配算法 | MG1×0.8 + MG2×1.0 + MG3×1.2 | score = min(1.0, m/4)，仅行业类 MG 关键词 |
...
├── data/
│   └── new_tag.csv            # 行业类关键词打标签数据（1413个，MG/NM分类）
└── results/
...
| `match_class` | string | `"MG"` / `"NM"` | 行业类关键词的打标分类；非行业类为 null |
| `grade` | integer | 1 / 2 / 3 | 泛化程度分级（1=泛化 / 2=适中 / 3=专精）；非行业类为 null |
...
行业类关键词已通过 `new_tag.csv` 手工打标（1413 个），分为两大类六级：
...
| NM1 | NM | 无匹配意义 | — | 不参与匹配 |
| NM2 | NM | 无匹配意义（业务型噪音） | — | 不参与匹配 |
| NM3 | NM | 无匹配意义（概念型噪音） | — | 不参与匹配 |

### 6.2 核心公式

仅使用**行业类且 `match_class = 'MG'`** 的关键词参与匹配（NM 和纯概念关键词排除）。

```
base_weight:  boosted(跨类别) = 2.0,  普通行业 = 1.0
grade_factor: MG1 = 0.8,  MG2 = 1.0,  MG3 = 1.2
final_weight = base_weight × grade_factor

effective_m = Σ(命中关键词的 final_weight)
score = min(1.0, effective_m / 4.0)
```

### 6.3 权重对比表

| 关键词类型 | 基础权重 | grade | 最终权重 |
|-----------|:-------:|:-----:|:--------:|
| MG1 × boosted | 2.0 | 0.8 | 1.6 |
| MG1 × 普通行业 | 1.0 | 0.8 | 0.8 |
| MG2 × boosted | 2.0 | 1.0 | 2.0 |
| MG2 × 普通行业 | 1.0 | 1.0 | 1.0 |
| MG3 × boosted | 2.0 | 1.2 | 2.4 |
| MG3 × 普通行业 | 1.0 | 1.2 | 1.2 |
| NM (任一) | — | — | 不参与 |
| 纯概念关键词 | — | — | 不参与 |
...
|:-----------:|:-----:|------|:-----------:|
| 0.8 | 0.20 | 低相关 | 1 个 MG1 行业命中 |
| 1.0 | 0.25 | 低相关 | 1 个 MG2 行业命中 |
| 1.6 | 0.40 | 中等 | 1 个 MG1 boosted，或 2 个 MG2 行业 |
| 2.0 | 0.50 | 中等 | 1 个 MG2 boosted |
| 3.0 | 0.75 | 较强相关 | 1 个 MG2 boosted + 1 个 MG2 行业 |
| 4.0 | 1.00 | 强相关 | 2 个 MG2 boosted，或 1 个 MG3 boosted + 2 个 MG2 行业 |

### 6.5 名称/代码兜底匹配

快讯匹配中，若股票名称或 6 位数字代码在文章中出现，直接判定为 **score=1.0**，无需关键词匹配。

若仅通过关键词匹配，则在 raw_score 基础上额外 **×0.75** 折扣，再与阈值 0.3 比较。
...
1. 从 Neo4j 获取该股票的所有行业类 MG 关键词（带最终权重）
2. 逐个关键词在 article_text 中做子串匹配
3. 命中 ⇒ 累加该关键词的 final_weight 到 effective_m
4. score = min(1.0, effective_m / 4.0)
5. 未达到阈值 / 纯关键词匹配额外 ×0.75
6. 返回 score
```
...
- `_get_matching_keywords(stock_code)` → list[{keyword, weight}]
  - 从 Neo4j 查询行业类 MG 关键词，按 MG1/MG2/MG3 分配最终权重
...
**行业类关键词打标**：
- 所有行业类关键词通过 `new_tag.csv` 手工打标（1413 个），分为 MG（有匹配意义）和 NM（无匹配意义）
- MG 细分三级：MG1（泛化，权重×0.8）/ MG2（适中，权重×1.0）/ MG3（专精，权重×1.2）
- NM 不参与任何匹配，NM1=过于泛化、NM2=业务型噪音、NM3=概念型噪音
- 非行业类关键词（概念/地区/主题）`match_class` 和 `grade` 均为 null，不受影响

**匹配规则细节**：
- 仅行业类且 `match_class = 'MG'` 的关键词参与匹配
- 非行业类关键词（纯概念/地区/主题）不参与匹配
- 跨类别关键词（同时有"概念"+"行业"）基础权重翻倍
- grade 倍率在基础权重之上叠加：MG1×0.8、MG2×1.0、MG3×1.2
- 匹配采用简单的子串包含判断（`keyword in article`），无分词/NLP
- 匹配度 score 最大为 1.0（当 effective_m ≥ 4 时）
- 股票名称/代码命中直接 score=1.0，纯关键词匹配额外 ×0.75 折扣

**标签更新方式**：
- 编辑 `knowledge_graph/data/new_tag.csv` 中的 `tag` 列
- 运行标签同步脚本：读取 csv → 清空所有 Keyword 的 match_class/grade → 仅对行业类关键词重新打标
- 非行业类关键词不受影响（属性保持 null）
...
| 日期 | 版本 | 变更 | 说明 |
|------|:----:|------|------|
| 2026-07-23 | v1.1 | 关键词打标体系 | 行业类1413个Keyword新增 `match_class` + `grade` 属性；MG1/2/3 三档倍率，NM 不参与匹配；新建 `new_tag.csv` 管理标签 |
| 2026-07-21 | v1.0 | 初始文档 | Neo4j 知识图谱全量构建 + 匹配算法文档 |
