...
| 试用档(2000积分) | **1次/分钟 + 10次/小时 + 10次/天**(三重限制, 请求到达即计数, 失败也算) |
| 正式档(8000积分) | 每天 100000 次, 10000 积分以上无总量限制 |
| 单次上限 | 3000 条, 可分页(offset/limit)循环 |
| 实测 | 区间 `start_date/end_date` 全市场拉取有效(2 周 3158 行/661 只); `month` 参数无效; 多值 `ts_code` 不支持 |
...
1. **用户明确允许**(口头/消息授权)
2. **额度可用**, 满足其一:
   - 试用档额度确认恢复(10次/小时 + 10次/天 可用), 或
   - 升级正式权限(8000 积分: 每天 100000 次, 无小时/天瓶颈)

### 5.2 重启步骤(建议顺序, 每步都先与用户确认)

1. **单次探测**(1 次调用): 拉近 6 个月全市场(`start_date=20260206, end_date=今天`),
   看返回行数与日期覆盖 → 判断是否 3000 截断
...
3. **回填近 6 个月**: 每天 ≤ 8 次调用(留 2 次余量给增量/重试), 分 2 天
   ...
   - 幂等: INSERT OR IGNORE, 中断可重跑
4. **恢复 22:00 cron 增量**: 加回 §4.3 的 cron 行(每天 1 次, 全市场当天)
