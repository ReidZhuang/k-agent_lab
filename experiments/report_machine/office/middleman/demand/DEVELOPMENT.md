...
| **v6 (异步搜索)** | **`/search` 立即返回 processing → `/poll` 轮询结果，后台线程搜索** |
...
客户端                           mail_tower
  │                                  │
  ├─ POST /search ─────────────────▶ │
  │   (query, engine, mode=list)     │  api.py handler
  │                                  ├─ session_manager.create()
  │                                  ├─ threading.Thread(target=_run_list_search_in_thread)
  │   ◀──── 200 ───────────────────  │
  │   {"status": "processing",       │  ← 立即返回
  │    "session_id": "s_..."}        │
  │                                  │
  │          [后台线程开始搜索]        │  _run_list_search_in_thread()
  │                                  ├─ SEARCH_SEM.acquire(timeout=300)
  │                                  ├─ loop.run_until_complete(run_search_pipeline())
  │                                  ├─ session_manager.set_preview()
  │                                  ├─ 启动各引擎正文提取线程
  │                                  └─ SEARCH_SEM.release()
  │                                  │
  ├─ GET /poll/{session_id} ───────▶ │  /poll 端点
  │   ◀──── 200 ───────────────────  │
  │   {"status": "list_ready",       │  ← 搜索结果已就绪
  │    "preview": {"articles": [...]}}│
  │                                  │
  ├─ POST /article ────────────────▶ │  /article 端点
  │   (取正文)                       │
```
...
│  api.py                                          │
│  ┌─────────────────────────────────────────────┐  │
│  │ POST /search → 创建 session → 启动后台线程  │  │  ← 立即返回 processing
│  │                   ↓                         │  │
│  │  _run_list_search_in_thread(线程)            │  │  ← SEARCH_SEM 限流
│  │    └─ run_search_pipeline()                 │  │
│  │    └─ set_preview() → 启动正文提取线程       │  │
│  ├─────────────────────────────────────────────┤  │
│  │ GET /poll/{session_id} → 返回当前 session 状态│  │
│  ├─────────────────────────────────────────────┤  │
│  │ POST /article → 获取正文                     │  │
│  └─────────────────────────────────────────────┘  │
│                                                    │
│  core.py — run_search_pipeline, 提取链             │
│  session_manager.py — 线程安全会话管理 + 文件持久化  │
│  search_engine/ — 各后端引擎                       │
│  sinafin_rate_limiter.py — 1.8s 跨进程节流         │
└──────────────────────────────────────────────────┘
```
...
| 模块 | 职责 |
|------|------|
| `api.py` | FastAPI 路由，请求/响应模型，后台线程管理，sinafin 按需加载，`/poll` 端点 |
...
原版 `/search` 是同步的——客户端发请求，服务端等搜索引擎返回后才响应。如果在搜索引擎执行期间连接断开，客户端拿不到 `session_id`，也无法查询进度。

v6 改为：**`/search` 立即返回 `session_id` + `status:processing`，搜索在后台线程执行，客户端通过 `/poll/{session_id}` 轮询结果。**

### 数据流

```
POST /search (mode=list)
  │
  ├─ session_manager.create() → 获得 session_id
  │
  ├─ threading.Thread(target=_run_list_search_in_thread)
  │     └─ 立即返回 {status:"processing", session_id}
  │
  └─ [后台线程] _run_list_search_in_thread()
        │
        ├─ SEARCH_SEM.acquire(timeout=300)   ← threading.Semaphore(16)
        │     超时 → set_error("服务繁忙")
        │
        ├─ loop = asyncio.new_event_loop()
        │   loop.run_until_complete(
        │     asyncio.wait_for(run_search_pipeline(...), timeout=90)
        │   )
        │     超时 → set_error("搜索超时")
        │
        ├─ set_preview(session_id, preview_data, raw_results)
        │
        ├─ 空结果 → 直接返回（/poll 看到 empty:true）
        │
        ├─ 启动各引擎正文提取线程（ddg/juchao/baidufin/thsfin/dcfin/qnainfo）
        │
        ├─ SEARCH_SEM.release()
        │
        └─ 异常 → set_error() + close()
...
`GET /poll/{session_id}` 返回 session 的当前状态：

| status | 含义 |
|:-------|:------|
| `processing` | 后台搜索还在进行中 |
| `list_ready` | 文章列表已就绪，正文待提取 |
| `done` | 全部完成（ddg/qnainfo 的正文已就绪） |
| `error` | 搜索失败，`error` 字段含原因 |

### 关键改动

| 文件 | 改动 |
|:-----|:------|
| `api.py` | `SEARCH_SEM` 改为 `threading.Semaphore(16)` |
| `api.py` | 新增 `_run_list_search_in_thread()` 后台搜索函数 |
| `api.py` | list 模式 handler 简化为：创建 session → 启动线程 → 立即返回 |
| `api.py` | 空结果不再关 session（客户端通过 `/poll` 看到 `empty:true`） |

### 并发控制

后台搜索使用 `threading.Semaphore(16)` 限制并发数：
- 每个 worker 进程最多 16 个后台搜索线程同时运行
- 超过的排队等待，最多等 300s（`MAX_SEARCH_WAIT`）
- 超时后 session 标记为 `error`，客户端通过 `/poll` 获知
- 搜索完成后由线程自己 release 槽位
