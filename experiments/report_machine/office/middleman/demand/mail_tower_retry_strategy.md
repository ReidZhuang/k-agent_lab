...
| 理由 | processing 不消耗 `body_return_count`，随便重试多少次都不影响。5s 固定间隔给后台足够的完成时间。120 次 × 5s = 600s（10 分钟），远超所有引擎的最大提取耗时。 |
...
4. **`/article` 的 `processing` 不消耗 `body_return_count`** — 重试 120 次也不影响
